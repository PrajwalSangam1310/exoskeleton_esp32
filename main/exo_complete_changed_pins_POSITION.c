...
//  digital pins
#define BLDC_PWM_U_GPIO 25
#define BLDC_PWM_V_GPIO 26
#define BLDC_PWM_W_GPIO 27
...
// inhibit pins
#define BLDC_EN_U_GPIO 14
#define BLDC_EN_V_GPIO 12
#define BLDC_EN_W_GPIO 13

// analog pins
#define HALL_CAP_U_GPIO 36
#define HALL_CAP_V_GPIO 39
#define HALL_CAP_W_GPIO 34
...
{
    static uint32_t past_counter_value;
    static uint32_t current_counter_value;
    ...
    float position_error = received_payload.goal_position - get_angle_from_counter(current_counter_value);
    test_feedback_payload.joint_angle = get_angle_from_counter(current_counter_value);
    position_error_i += position_error;
    printf("position error is: %f\n", position_error);
    ...
    float duty_change = KP_VELOCITY * velocity_error + KI_VELOCITY * velocity_error_i;
    new_duty += duty_change;
    duty = (int)new_duty;
    if (duty < 0)
    {
    ...
    printf("The calculated duty_change is: %f\n", duty_change);
    printf("The calculated duty is: %d\n", duty);
}
